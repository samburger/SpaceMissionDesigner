...
thruster_mass = 125       # kg, Mass of the thruster itself.
isp = 1800                # sec
delta_v = 1470+496.72+668.46           # m/sec
contingency_factor = 1.1
ox_fuel_ratio = 0     # Ratio of bipropellant oxidizer to fuel, by mass.  0 if monopropellant.
m_f_initial = 2500    # kg, All mass above this stage - the "first guess" for dry mass.


# Prop Tank Sizing Inputs
density_fuel = 1145    # kg/m^3
density_ox = 1.0      # kg/m^3, set 1 if monopropellant
aspect_ratio = 3.5
mass_factor = 1.25
...
while abs(m_f_sph - m_f_last_sph) > 0.01:
    ...
    radius_fuel_sph = ( volume_fuel/(math.pi*4/3.0) ) ** (1/3.0)
    radius_ox_sph = ( volume_ox/(math.pi*4/3.0) ) ** (1/3.0)
    ...
    tank_mass_fuel_sph = mass_factor*wall_density*(4/3.0*math.pi*((radius_fuel_sph+thickness_fuel_sph)**3-radius_fuel_sph**3))
    tank_mass_ox_sph = mass_factor*wall_density*(4/3.0*math.pi*((radius_ox_sph+thickness_ox_sph)**3-radius_ox_sph**3))
...
while abs(m_f_cyl - m_f_last_cyl) > 0.01:
    ...
    radius_fuel_cyl = ( volume_fuel/(math.pi*(aspect_ratio+4/3.0)) ) ** (1/3.0)
    radius_ox_cyl = ( volume_ox/(math.pi*(aspect_ratio+4/3.0)) ) ** (1/3.0)
    ...
    thickness_fuel_cyl_caps = thickness_fuel_cyl/2.0
    thickness_ox_cyl_caps = thickness_ox_cyl/2.0

    tank_mass_fuel_cyl = mass_factor*wall_density*math.pi*(4/3.0*((radius_fuel_cyl+thickness_fuel_cyl_caps)**3-radius_fuel_cyl**3)+aspect_ratio*radius_fuel_cyl*(2*radius_fuel_cyl*thickness_fuel_cyl+thickness_fuel_cyl**2))
    tank_mass_ox_cyl = mass_factor*wall_density*math.pi*(4/3.0*((radius_ox_cyl+thickness_ox_cyl_caps)**3-radius_ox_cyl**3)+aspect_ratio*radius_ox_cyl*(2*radius_ox_cyl*thickness_ox_cyl+thickness_ox_cyl**2))
